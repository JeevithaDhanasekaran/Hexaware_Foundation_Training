...
import com.hexaware.carrental.service.implementations.LeasesServiceImpl;
import com.hexaware.carrental.service.implementations.PaymentsServiceImpl;
...
public class CustomerMenu {
	...
	private void makePayment() throws InvalidInputException, DatabaseConnectionException {
		...
		// Get expected amount to be paid
		...
		try {
			...
			if (result) {
				System.out.print("Please rate your vehicle experience (0 to 5): ");
				double rating = scanner.nextDouble();

				if (rating < 0 || rating > 5) {
					System.out.println("Invalid rating. Must be between 0 and 5. Skipping rating update.");
				} else {
					int vehicleId = vehiclesService.getVehicleIdByLeaseId(leaseId);
					boolean ratingUpdated = vehiclesService.updateVehicleRating(vehicleId, rating);

					if (ratingUpdated) {
						System.out.println("Thank you! Your rating has been submitted.");
					} else {
						System.out.println("Failed to update rating. Please try again later.");
					}
				}
			}
	...
	private void createLease() throws InvalidInputException, DatabaseConnectionException, LeaseNotFoundException {
		...
		leasesService.createLease(customerId, vehicleId, java.sql.Date.valueOf(start), java.sql.Date.valueOf(end),
				leaseType);
		vehiclesService.updateCarStatus(vehicleId, "BOOKED");
		vehiclesService.getCarStatusByVehicleId(vehicleId);
